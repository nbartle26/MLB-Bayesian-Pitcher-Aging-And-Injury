...
output_dir <- "C:/Users/Owner/Desktop/Saberseminar2026"
...
base_seasons <- pitcher_data_final %>%
  filter(!is_missing, year <= max(year) - 3) %>%
  select(player_id, year, velocity, tj_count)

future_min_tbl <- base_seasons %>%
  left_join(
    pitcher_data_final %>%
      filter(!is_missing) %>%
      select(player_id, fy = year, fv = velocity),
    by = "player_id", relationship = "many-to-many"
  ) %>%
  filter(fy > year, fy <= year + 3) %>%
  group_by(player_id, year) %>%
  summarise(future_min = min(fv), .groups = "drop")

future_outcomes <- base_seasons %>%
  left_join(future_min_tbl, by = c("player_id", "year")) %>%
  mutate(
    tj_bucket = pmin(tj_count, 2),
    exited    = is.na(future_min),
    big_drop  = !exited & (velocity - future_min) > 1.5,
    bad       = as.numeric(exited | big_drop)
  )

injury_risk_lookup <- future_outcomes %>%
  group_by(tj_bucket) %>%
  summarise(n = n(), bad_rate = mean(bad), .groups = "drop") %>%
  mutate(injury_risk_value = bad_rate)

print(injury_risk_lookup)
...
risk_assessments <- pitcher_data_final %>%
  ...
  left_join(
    ...
    by = "player_id") %>%
  mutate(tj_bucket = pmin(tj_count, 2)) %>%
  left_join(
    injury_risk_lookup %>% select(tj_bucket, tj_risk = injury_risk_value),
    by = "tj_bucket") %>%
...
# Model validation 
validation_data <- modeling_data %>% filter(year <= 2025)  # Split data for validation
train_data <- validation_data %>% filter(year <= 2023)
test_data <- validation_data %>% filter(year >= 2024)
...
#Case Study Examples
case_study_names <- c("Hamels, Cole", "Wainwright, Adam", "Taillon, Jameson", "deGrom, Jacob", "Buehler, Walker", "May, Dustin", "Liberatore, Matthew", "Pallante, Andre")
...
create_projection_final <- function(pitcher_name_input, pitcher_data, future_preds, risk_data) {
  ...
  bridge <- data.frame(
    age = actual_data$age[nrow(actual_data)],
    velocity = actual_data$velocity[nrow(actual_data)],
    lower = actual_data$velocity[nrow(actual_data)],
    upper = actual_data$velocity[nrow(actual_data)]
  )
  future_data <- bind_rows(bridge, future_data)
...
projection_names <- c("Brown, Hunter", "Cabrera, Edward", "Pallante, Andre", "May, Dustin", "Liberatore, Matthew")
...
p_performance <- ggplot(performance_data, aes(x = actual, y = predicted)) +
  ...
  labs(title = "Model Validation: Predicted vs Actual",
       subtitle = "Held-out test set (2024-2025)",
...
p_smooth <- ggplot(aging_curve_clean, aes(x = age)) +
  ...
  labs(title = "MLB Pitcher Velocity Aging Curve (2015-2025)",
       subtitle = "Smoothed population trajectory with 95% intervals",
       x = "Age (years)",
...
cardinals_names <- c("Wainwright, Adam", "May, Dustin", "Pallante, Andre", "Liberatore, Matthew", "Gray, Sonny")
for(name in cardinals_names) {
  cat("Generating:", name, "\n")
  p <- create_case_study_final(name, pitcher_data_final, velocity_model, 
                               changepoint_results, risk_assessments)
  if(!is.null(p)) {
    filename <- paste0(output_dir, "case_study_", 
                       gsub(", ", "_", tolower(name)), "_cardinals.png")
    ggsave(filename, p, width = 10, height = 6, dpi = 300)
    cat("  Saved\n")
  } else {
